...
- **Digicom NI prove (2026-08-08)** — success floor = `ni-parked` → `sri-ok msc=…` → `MapNiPush ni-sent notify` + log `USSD NI sent … mscGt=… imsi=…`; pcap CalledParty = MSC GT. Peer **`unstructuredSSNotify_Response`** = network accepted Notify (still ≠ handset UI proof). Fresh proof: [`build/pcap/ussd-ni-push-msc-20260808-025117.pcap`](build/pcap/ussd-ni-push-msc-20260808-025117.pcap) (MSISDN **251911230398**, HLR GT **251900000006**, MSC CalledParty **251971200146**, Notify + peer response). Prior: `ussd-ni-push-msc-20260808-023952.pcap`. → [ss7-lab-pair](docs/agents/ss7-lab-pair.md)
...
- **5k TPS honesty** — Digicom live heap **2g/4g** on ~15 GiB (OTA co-resident); pool knobs already ×10 (`sbb-pool-max=40960`, `buffer-size=16384`, HTTP worker **256**, client pool **2048**) but **BUILD_TIME** (re-package). Dual SCTP 2011/2019 helps link share, not automatic 5k. Honest floor today = functional NI/MO + knobs baked; **5k not measured on Digicom** — need dedicated load host (≥8g) + map/load + AS sim before claiming. → [lessons](docs/agents/lessons.md)
