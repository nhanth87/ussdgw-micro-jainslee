...
- **Tenant `network_id` ≡ SCCP `networkId`** — Digicom stack GTT is **networkId=0** only. Tenant `digicom-push` must be **0**; `network_id=1` → SCCP `no matching Rule` → 0 SCTP DATA + `SRI_TIMEOUT` despite `SRI-SM sent`. → [lessons](docs/agents/lessons.md)
...
- **Digicom NI prove (2026-08-08)** — Notify floor = `ni-parked` → `sri-ok msc=…` → `ni-sent notify` + `USSD NI sent … mscGt=… imsi=…`; pcap CalledParty = **MSC GT** (not MSISDN). Peer Notify RESULT ≠ handset UI. Wireshark column **E.164 (MSISDN)** on Notify often shows **Calling GW GT** (`251971200490`) — ignore; use SCCP **Called Party Digits** = MSC. Pcaps: `ussd-hello-world-SRI-20260808-031853.pcap`, `ussd-ni-push-msc-20260808-025117.pcap` (MSISDN **251911230398**). Prefer live prove with **Request** when handset must display a menu. → [ss7-lab-pair](docs/agents/ss7-lab-pair.md)
...
- **5k TPS honesty** — Digicom live heap **2g/4g** on ~15 GiB (OTA co-resident); pool knobs ×10 (`sbb-pool-max=40960`, `buffer-size=16384`) are **BUILD_TIME**. Runtime targets for sync AS: HTTP worker **512**, client pool **8192**, JDBC **128/16**, CDR queue **100k**/batch **2k**. Dual SCTP 2011/2019 helps link share, not automatic 5k. Honest floor = path prepared; **5k not measured on Digicom** — need dedicated load host (≥8g) + map/load + AS sim. → [lessons](docs/agents/lessons.md)
...
- **Digicom host = prod-bound** — live **Balance Plus** peer + **PostgreSQL** dedicated DB **`ussdgw`** (never OTA’s `ota`). Not a disposable toy lab. Package with build-time **`db-kind=postgresql`**, then restore local **H2** for the dev tree. Rsync **jars/`lib`/`quarkus`/`app/html` only** — **never** overwrite Digicom `configs/` (`application.properties`, `ss7-digicom-balance.json`, persist). → [schema](docs/agents/schema.md) · [ss7-lab-pair](docs/agents/ss7-lab-pair.md)
...
- **MO Called SSN 147 (gsmSCF)** — Balance Plus may address Digicom with SSN **147**; seed `services` must include `gsmscf:147` (+8/+6) or SCCP UDTS. Prove boot `Registered SCCP listener with extra ssn 147`. → [ss7-lab-pair](docs/agents/ss7-lab-pair.md) · [lessons](docs/agents/lessons.md)
