...
- **Dashboard KPI + CDR digest** — KPI values = human short format (never raw `Map.toString()`); **all** `.metric-card`s overflow-safe. CDR primary = AS USSD (~50) + outcome (END/CONTINUE/fail); expand = **status of all services** (MAP/HLR/Bridge/AS/UE) via `CdrServiceStatuses` — never `GATE_ARMED` / VirtualSessionBridge pipe as hero; raw detail under Advanced. Never `|`→`/` escape of `events_json` detail. → [skills § Admin](docs/agents/skills.md) · [lessons](docs/agents/lessons.md)
